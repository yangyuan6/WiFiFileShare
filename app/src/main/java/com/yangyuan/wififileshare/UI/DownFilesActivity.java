...
import com.yangyuan.wififileshare.Trans.ClientGetFileListHandler;
import com.yangyuan.wififileshare.Trans.ClientGetHalfFileHandler;
import com.yangyuan.wififileshare.UI.UIUtils.FlikerProgressBar;
import com.yangyuan.wififileshare.Utils.DowningFileInfoFromSdUtil;
import com.yangyuan.wififileshare.Utils.FileSizeFormatUtil;
import com.yangyuan.wififileshare.Utils.GetFileReceiveHistoriesUtil;
import com.yangyuan.wififileshare.Utils.GetServiceFileInfosFromSdUtil;
import com.yangyuan.wififileshare.Utils.IcoUtil;
import com.yangyuan.wififileshare.Utils.Ipint2StringUtil;
import com.yangyuan.wififileshare.Utils.SaveFileInfo2SdUtil;
import com.yangyuan.wififileshare.Utils.SaveFileReceiveHistories2SdUtil;
 ...
 * 下载文件界面
 */
public class DownFilesActivity extends Activity implements View.OnClickListener, SendStateChangedReciver.OnSendStateChangedListener {
    ...
    private ClientGetFileListHandler clientGetFileListHandler;
    private ClientGetHalfFileHandler clientGetHalfFileHandler;
    private ArrayList<FileReceiveHistory> fileReceiveHistories;
    private ArrayList<ServiceFileInfo> shareFileInfo;
    private String uuid;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                            ...
                            for (int i = 0; i < downFiles.size(); i++) {
                                if (uuid.equals(downFiles.get(i).getUuid())) {
                                    ...
                                    if (tag == 0) {
                                        ...
                                        shareFileInfo.add(serviceFileInfo1);
                                    }
                                }
                            }
                        }
                ...
                case AllFINISH: {
                    ...
                    SaveFileInfo2SdUtil.save(shareFileInfo);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        shareFileInfo = GetServiceFileInfosFromSdUtil.doAction();
        initDateAndView();
        taskInit();
    }
    ...
    public void taskInit() {
        ...
        if (downhalffile ==1) {
            clientGetHalfFileHandler = new ClientGetHalfFileHandler(ip, downFiles, uiHandler);
            clientGetHalfFileHandler.setPriority(Thread.MAX_PRIORITY);
            clientGetHalfFileHandler.start();
        } else {
            clientGetFileListHandler = new ClientGetFileListHandler(ip, downFiles, uiHandler);
            clientGetFileListHandler.setPriority(Thread.MAX_PRIORITY);
            clientGetFileListHandler.start();
        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.iv_back:
                ...
                DownFilesActivity.this.onDestroy();
                this.finish();
                break;
            case R.id.bottomButton: {
                if (bottomButton.getText().equals("取消")) {
                    ...
                    } else if (clientGetHalfFileHandler != null) {
